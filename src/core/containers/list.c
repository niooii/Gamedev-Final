...
void* __list_create(u64 length, u64 stride) {
    ...
    u64* new__list = GDF_Malloc(header_size + list_size, GDF_MEMTAG_LIST);
    new__list[LIST_CAPACITY] = length;
    new__list[LIST_LENGTH] = 0;
    new__list[LIST_STRIDE] = stride;
    return (void*)(new__list + LIST_FIELD_LENGTH);
}
